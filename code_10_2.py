...
def function(x):
    return torch.mean(x[:, :D-1] * x[:, 1:D], dim=1) + \
           2 * torch.mean(torch.sin(2 * np.pi * x) * (x < 0.5), dim=1) + \
           torch.mean((4 * np.pi / (np.sqrt(2) - 1) * (x - 1 / np.sqrt(2))**2 - np.pi * (np.sqrt(2) - 1)) * (x > 0.5), dim=1)
...
torch.save(participating_error, 'e_1_p_error_30_10.pt')
torch.save(non_participating_error, 'e_1_np_error_30_10.pt')